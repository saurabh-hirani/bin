...
cust_func(){
  aws sns get-topic-attributes --topic-arn $2 --query 'Attributes.Policy' | jq '. | fromjson | .Statement[].Principal' | grep -v null > /var/tmp/sns/topic-policy-status/$1.txt 2>&1
}

target_topic=$1
mkdir -p /var/tmp/sns/topic-policy-status
