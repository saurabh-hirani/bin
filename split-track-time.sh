...
update_remaining_minutes() {
  ...
  for i in $(seq 0 $duration_mins); do
    remaining_time=$(echo "$local_duration_mins - $i" | bc)
    osascript -e 'Tell application "AnyBar_copy" to set title to "remaining_time='$remaining_time/$local_duration_mins' mins"'
    ((i++))
    sleep 60
  done
}
update_remaining_minutes $duration_mins &
...
osascript -e 'Tell application "AnyBar_copy" to set title to "remaining_time=0 mins"'
